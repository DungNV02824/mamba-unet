...
Architecture chính xác theo paper
"""
import torch
...
class EncoderStage(nn.Module):
    """
    Encoder Stage: VSS Block ×N → Patch Merging
    """
...
class MambaUNet(nn.Module):
    ...
    def __init__(
        ...
        num_classes=2,
        embed_dim=96,
        depths=[2, 2, 2, 2],  # Số VSS blocks mỗi stage
        drop_path_rate=0.2,
        patch_size=4,
    ):
        super().__init__()
        ...
        # ==================== FINAL LAYERS ====================
        # Upsample cuối cùng về kích thước gốc
    ...
    def forward(self, x):
        ...
        """
        # ==================== PATCH PARTITION ====================
        x = self.patch_partition(x)  # (B, C, H/4, W/4)
        ...
        # ==================== DECODER ====================
        # Bỏ skip connection cuối (không dùng cho decoder)
...
def create_mamba_unet(in_chans=1, num_classes=2, img_size=512):
    ...
    model = MambaUNet(
        ...
        embed_dim=96,
        depths=[2, 2, 2, 2],  # VSS blocks per stage
        drop_path_rate=0.2,
        patch_size=4
    )
    return model